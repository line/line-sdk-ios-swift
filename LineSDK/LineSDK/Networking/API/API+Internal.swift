...
extension API {
    ...
    {
        let error = LineSDKError.generalError(reason: .deprecatedAPI(apiName: "sendMessages"))
        queue.execute { completion(.failure(error)) }
    }
    ...
    {
        let error = LineSDKError.generalError(reason: .deprecatedAPI(apiName: "multiSendMessages"))
        queue.execute { completion(.failure(error)) }
    }
}
...
extension API {
    ...
    {
        let error = LineSDKError.generalError(reason: .deprecatedAPI(apiName: "getMessageSendingOneTimeToken"))
        queue.execute { completion(.failure(error)) }
    }

    @available(*, deprecated, message: "Sharing API is no longer supported.")
    public static func getMessageSendingOneTimeToken(userIDs: [String]) async throws -> MessageSendingToken {
        throw LineSDKError.generalError(reason: .deprecatedAPI(apiName: "getMessageSendingOneTimeToken"))
    }
    ...
    {
        throw LineSDKError.generalError(reason: .deprecatedAPI(apiName: "multiSendMessages"))
    }
}

// MARK: - OpenChat Related API
extension API {
